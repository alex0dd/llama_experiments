...
class ModelParser:
    ...
    def tensor_names(self) -> List[str]:
        return list(
            chain(*[parser.tensor_names for parser in self._parsers_dict.values()])
        )

    @property
    def tensor_shapes(self) -> List[Tuple[str, List[int]]]:
        return list(
            chain(*[parser.tensor_shapes for parser in self._parsers_dict.values()])
        )

    def get_tensor(self, tensor_name: str) -> np.array:
        assert (
            tensor_name in self.tensor_names
        ), f"Tensor {tensor_name} is not present in loaded tensors"
    ...
    def get_tensors(self, tensor_names: List[str]):
        ...
        for parser in parser_tensor_read_map.keys():
            tensors_dict = parser.get_tensors_without_closing_file(
                parser_tensor_read_map[parser]
            )
            output_dict.update(tensors_dict)
        return output_dict

    def _build_tensors_parsers_map(
        self, parsers_dict: Dict[str, SafeTensorsParser]
    ) -> Dict[str, SafeTensorsParser]:
        """
