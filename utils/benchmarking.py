...
class PerformanceMonitor:
    ...
    def __enter__(self):
        ...
        self.peak_memory = psutil.Process().memory_info().rss / (1024 * 1024 * 1024)  # GB
        if torch.cuda.is_available():
            torch.cuda.reset_peak_memory_stats()
            self.cuda_peak_memory = torch.cuda.max_memory_allocated() / (1024 * 1024 * 1024)  # GB
        return self
    ...
    def record_token_generation(self, num_tokens: int):
        ...
        if torch.cuda.is_available():
            current_cuda_memory = torch.cuda.max_memory_allocated() / (1024 * 1024 * 1024)
